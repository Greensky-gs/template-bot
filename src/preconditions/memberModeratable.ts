...
export default new Precondition('memberModeratable').setChatInputRun(({ interaction, options }) => {
    ...
    const member = options.getMember(
        options.data.filter((x) => x.type === ApplicationCommandOptionType.User)[0]?.name
    ) as GuildMember;
    ...
    if (member.roles.highest.position >= member.guild.members.me.roles.highest.position)
        return obj(preconditionData('memberModerable').userTooHighBot);
    ...
    if (member.roles.highest.position >= (interaction.member as GuildMember).roles.highest.position)
        return obj(preconditionData('memberModerable').userTooHighMod);
