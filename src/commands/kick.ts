...
export default new Command({
  ...
  run: async ({ interaction, args }) => {
    ...
    await member
      .send(kickMsgToMember(interaction.user, interaction.guild.name))
      .catch(() => {});
    const result = await member
      .kick(`Kicked by ${interaction.user.tag} ( ${interaction.user.id} )`)
      .catch(() => {
        interaction
          .editReply(kickError(interaction.user, member.user))
          .catch(() => {});
      });

    if (result)
      interaction
        .editReply(kick(interaction.user, member.user))
        .catch(() => {});
  },
});
