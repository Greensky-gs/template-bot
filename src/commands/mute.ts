...
export default new Command({
  ...
  run: async ({ interaction, args }) => {
    ...
    await member
      .send(muteMsgToMember(interaction.user, interaction.guild.name, time))
      .catch(() => {});
    const result = await member.timeout(time, reason).catch(() => {
      interaction
        .editReply(muteError(interaction.user, member.user))
        .catch(() => {});
    });
    if (result)
      interaction
        .editReply(mute(interaction.user, member.user, time))
        .catch(() => {});
  },
});
