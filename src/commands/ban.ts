...
export default new Command({
  ...
  run: async ({ interaction, args }) => {
    ...
    await member
      .send(banMsgToMember(interaction.user, interaction.guild.name))
      .catch(() => {});
    const result = await member
      ...
      .catch(() => {
        interaction
          .editReply(banError(interaction.user, member.user))
          .catch(() => {});
      });

    if (result)
      interaction.editReply(ban(interaction.user, member.user)).catch(() => {});
  },
});
