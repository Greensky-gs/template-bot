...
export default new Event('interactionCreate', (inter) => {
    ...
    if (interaction.isCommand()) {
        ...
        if (!command) return interaction.reply(Object.assign(unkownCommand(interaction.user), { ephemeral: true })).catch(() => {});
        ...
        if (command.permissions) {
            ...
            if (missing.length > 0) return interaction.reply(Object.assign(missingPerms(interaction.user, missing), { ephemeral: true })).catch(() => {});
        };
        command.run({
            interaction,
