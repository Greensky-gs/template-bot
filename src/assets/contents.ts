...
export function unkownCommand(user: User) {
    return generateData(
        basic(user).setTitle('Unknown command').setDescription(`This command doesn't exist`).setColor('#ff0000')
    );
}
export function missingPerms(user: User, perms: permissionsArray) {
    return generateData(
        basic(user)
            ...
            .setDescription(
                `You need some permissions to execute this command.\n\nMissing : ${perms
...
export function memberIsOwner(user: User, member: User) {
    return generateData(
        basic(user).setTitle('Owner').setDescription(`<@${member.id}> is the server owner`).setColor('#ff0000')
    );
}
export function memberIsBot(user: User, member: User) {
    const embed = basic(user)
        .setTitle('Bot')
        .setColor('#ff0000')
        .setDescription(`<@${member.id}> is a bot.\nI cannot do this on a bot.`);

    return generateData(embed);
}
export function memberIsSelfUser(user: User) {
    return generateData(
        basic(user).setTitle('Self targetting').setDescription(`You're trying to moderate yourself`).setColor('#ff0000')
    );
}
...
export function muteMsgToMember(user: User, guildName: string, time: number) {
    return generateData(
        basic(user)
            .setTitle('Ban')
            .setDescription(
                `You've been banned from ${guildName} by ${user.tag} ( <@${user.id}> \`${
                    user.id
...
export function noBans(user: User) {
    return generateData(
        basic(user).setTitle('No bans').setDescription(`No member are banned from the server`).setColor('#00ff00')
    );
}
...
export function memberCount(user: User, stats: { humans: number; bots: number }) {
    return generateData(
        basic(user)
            ...
            .setDescription(
                `There is a total of **${(stats.bots + stats.humans).toLocaleString('en')} members in the server**`
            )
            .setFields(
                {
                    name: 'Bots',
...
export function feedback(user: User, feedback: string) {
    return generateData(basic(user).setTitle('Feedback').setDescription(feedback).setColor('#00ff00'));
}
...
export function cancel() {
    return generateData(new EmbedBuilder().setTitle('💡 Command canceled').setColor('Yellow'));
}
export function adminList(user: User, admins: Collection<string, GuildMember>) {
    return generateData(
        basic(user)
            .setTitle('Admin list')
            .setDescription(
                `Here are all the administrators that you want (${admins.size}):\n${admins
                    .map((x) => `<@${x.user.id}>`)
                    .join(' ')}`
            )
            .setColor('Orange')
...
export function reflex(user: User, time?: number | string) {
    if (!time) {
        return generateData(basic(user).setTitle('Reflex').setDescription(`Be ready to click...`).setColor('Yellow'));
    }
    if (time === 'start') {
        return generateData(basic(user).setTitle('Reflex').setDescription(`Click !`).setColor('#00ff00'));
    }
