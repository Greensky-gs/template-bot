...
export function banMsgToMember(user: User, guildName: string) {
  return generateData(
    basic(user)
      .setTitle("Ban")
      .setDescription(
        `You've been banned from ${guildName} by ${user.tag} ( <@${user.id}> \`${user.id}\` )`
      )
      .setColor("#ff0000")
  );
}
...
export function kickMsgToMember(user: User, guildName: string) {
  return generateData(
    basic(user)
      .setTitle("Kick")
      .setDescription(
        `You've been kicked from ${guildName} by ${user.tag} ( <@${user.id}> \`${user.id}\` )`
      )
      .setColor("#ff0000")
  );
}
...
export function muteMsgToMember(user: User, guildName: string, time: number) {
  return generateData(
    basic(user)
      .setTitle("Ban")
      .setDescription(
        `You've been banned from ${guildName} by ${user.tag} ( <@${
          user.id
        }> \`${user.id}\` ).\nYou'll be unmuted <t:${(
          (time + Date.now()) /
          1000
...
export function antispamTooFast(user: User) {
  return generateData(
    basic(user)
      .setTitle("Too fast")
      .setDescription(
        `<@${user.id}>, you're sending messages too fast.\n\nYou've been muted for 5 minutes`
      )
      .setColor("#ff0000")
  );
}
...
export function memberCount(
  user: User,
  stats: { humans: number; bots: number }
) {
  return generateData(
    basic(user)
      .setTitle("Membercount")
      .setDescription(
        `There is a total of **${(stats.bots + stats.humans).toLocaleString(
          "en"
...
export function cancel() {
  return generateData(
    new EmbedBuilder().setTitle("💡 Command canceled").setColor("Yellow")
  );
}
export function adminList(user: User, admins: Collection<string, GuildMember>) {
  return generateData(
    basic(user)
      .setTitle("Admin list")
      .setDescription(
        `Here are all the administrators that you want (${
          admins.size
...
export function reflex(user: User, time?: number | string) {
  ...
  return generateData(
    `You managed to click in ${time} milliseconds (that's nice)`
  );
}
